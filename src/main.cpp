...
// UInt128Hash enables using UInt128 as a key in unordered_map.
struct UInt128Hash {
...
int main() {
    ...
    std::cout << "Deterministically sorted forest nodes (by depth, then id):\n";
    ...
    const bool ok = verifySortedByDepthAndId(sorted);
    std::cout << "\nVerification of sorted vector: " << (ok ? "OK" : "FAILED")
              << "\n";
